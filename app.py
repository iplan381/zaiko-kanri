    ...
    if user_name != "-- 選択 --":
        ...
        for i, row in selected_data_list.iterrows():
            with st.expander(f"📌 {row['商品名']} ({row['サイズ']} / {row['地名']})", expanded=True):
                ...
                # 【修正：棚卸を追加】
                with col1: m_type = st.radio("操作区分", ["入庫", "出庫", "予約出庫", "棚卸(実数修正)"], horizontal=True, key=f"type_{i}")
                with col2:
                    m_qty = st.number_input("数量 / 実数", min_value=0, value=int(row['在庫数']) if m_type == "棚卸(実数修正)" else 0, key=f"qty_{i}")
                with col3:
                    if m_type == "予約出庫":
        ...
        if st.button("🔄 全ての変更を確定する", type="primary", use_container_width=True):
            ...
            def confirm_dialog(payloads):
                st.warning("以下の内容でデータを更新します。よろしいですか？")
                for idx, p in payloads.items():
                    action = f"【{p['type']}】 {p['qty']}" if not p['delete'] else "【削除】"
                    st.write(f"・{p['orig_data']['商品名']} ({p['orig_data']['サイズ']}/{p['loc']}) → {action}")
                
                if st.button("はい、更新します", type="primary", use_container_width=True):
